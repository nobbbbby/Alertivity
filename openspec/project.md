...
### Testing Strategy
- There are no automated unit/UI tests yet; verification is done manually by running the app, observing live metrics, and forcing CPU spikes to hit elevated/critical states
- After any code changes, run a local build (`xcodebuild -project Alertivity.xcodeproj -scheme Alertivity -configuration Debug -sdk macosx build`) and rerun until it succeeds before considering work complete
...
- After code changes, perform a debug build to ensure the app compiles and logging behaves before relying on manual runtime checks
